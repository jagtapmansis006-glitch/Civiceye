...
export function Brand({ className, subtitle = "City Services" }: { className?: string; subtitle?: string }) {
  return (
    <Link to="/" className={cn("flex items-center gap-2.5", className)} aria-label="CivicEye home">
      ...
      <span className="leading-none">
        ...
        <span className="block text-[10px] uppercase tracking-[0.18em] text-ink/50">{subtitle}</span>
      </span>
    </Link>
  );
}
